...
class WarnsMod(loader.Module):
    ...
    async def warnslimitcmd(self, message):
        """Установить лимит предупреждений. Используй: .warnlimit <кол-во:int>."""
        if not message.is_private:
            warns = self.db.get("Warns", "warns", {})
            args = utils.get_args_raw(message)
            chatid = str(message.chat_id)
    ...
    async def getwarnscmd(self, message):
        """Посмотреть кол-во варнов. Используй: .getwarns <@ или реплай> или <list>."""
        if not message.is_private:
            args = utils.get_args_raw(message)
            reply = await message.get_reply_message()
            chatid = str(message.chat_id)
    ...
    async def swarncmd(self, message):
        """Изменить режим ограничения. Используй: .swarn <kick/ban/mute>."""
        if not message.is_private:
            args = utils.get_args_raw(message)
            chatid = str(message.chat_id)
            warns = self.db.get("Warns", "warns", {})
        ...
        if not message.is_private:
            ...
            try:
                ...
                if len(warns[chatid][userid]) == 0:
                    warns[chatid].pop(userid)
                self.db.set("Warns", "warns", warns)
                await message.edit(f"<b>У <a href=\"tg://user?id={user.id}\">{user.first_name}</a> удалено последнее предупреждение.</b>")
            except KeyError:
                return await message.edit(f"<b>У <a href=\"tg://user?id={user.id}\">{user.first_name}</a> нет предупреждений.</b>")
        else:
            return await message.edit("<b>Это не чат!</b>")